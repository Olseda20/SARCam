...
class findSaliencyFineGrained():
      ...
      def getSaliency(self,img,threshperc):
            ...
            threshpercconv = int((threshperc/100)*255)
...
class findSaliencySpectralResidual():
      ...
      def getSaliency(self,img,threshperc):
            ...
            saliencyMap = np.array(255*self.saliency.computeSaliency(img)[1],dtype="uint8")
            # creating a threshold percentatge to grescale conversion
            thresh_perc_conv = int((threshperc/100)*255)
            # applying thresholding the to top 'threshperc' of the image
            threshMap = cv2.threshold(saliencyMap, thresh_perc_conv, 255, cv2.THRESH_BINARY)[1]
            return saliencyMap, threshMap

## Saliency Methods

def getContors(threshMap, orig):
...
def getContorPosition(img, boundingBoxes):
      #Left Right
      L, C, R = 0, 0, 0
      #Up Down
      T, M, B = 0, 0, 0
...
if __name__ == '__main__':
      ...
      t = 0 
      t0 =0
      findSaliency = findSaliencyFineGrained()
      findSaliency2 = findSaliencySpectralResidual()
      threshold = 85
      i = 0
